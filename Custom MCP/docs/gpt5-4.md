### Plan for the updated nmap tool

- **Critical fix — circuit breaker scope:**
  - Change instance shadowing to class-level reset when reinitializing the circuit breaker.

- **Medium fixes — argument handling and validation:**
  - Use shlex to robustly parse `extra_args`.
  - Align injected flags with `allowed_flags`; explicitly include `--max-parallelism`.
  - Enforce targets to be RFC1918, loopback, or end with `.lab.internal` to match the docstring.
  - Keep optimization flags before user args so user-specified flags later can override defaults.

- **Low-risk improvements for consistency and ops hygiene:**
  - Return timezone-aware UTC timestamps.

---

### Drop-in replacement: mcp_server/tools/nmap_tool.py
...
- **`--max-parallelism` support:** It’s included to keep your injected defaults aligned with `allowed_flags`. If your nmap build doesn’t support it, replace with a supported rate/hostgroup option and add that to `allowed_flags`.
- **RFC1918/loopback/`.lab.internal` enforcement:** This makes behavior match the docstring and protects from accidental scans outside controlled ranges.
- **User override precedence:** Defaults are prepended so any later user flags can override, keeping flexibility while providing safe baselines.

https://copilot.microsoft.com/shares/V6r7vAnQB9BkcABRVSEFM
